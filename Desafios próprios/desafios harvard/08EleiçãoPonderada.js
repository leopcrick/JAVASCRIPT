...
async function passo3(){
    ...
    for (i=0 ; i<numeroEleitores ; i++){
        const inputUsuario1 = await questionAsync(`Eleitor ${i+1} - número do candidato para preferência 1: `)
        const inputUsuario2 = await questionAsync(`Eleitor ${i+1} - número do candidato para preferência 2: `)
        const inputUsuario3 = await questionAsync(`Eleitor ${i+1} - número do candidato para preferência 3: `)
        const votoEleitor1 = Number(inputUsuario1)
        const votoEleitor2 = Number(inputUsuario2)
        const votoEleitor3 = Number(inputUsuario3)
        if (verificarVoto(votoEleitor1) && verificarVoto(votoEleitor2) && verificarVoto(votoEleitor3)){
            candidatosMAP.set(candidatos[votoEleitor1-1], candidatosMAP.get(candidatos[votoEleitor1-1])+3)
            candidatosMAP.set(candidatos[votoEleitor2-1], candidatosMAP.get(candidatos[votoEleitor2-1])+2)
            candidatosMAP.set(candidatos[votoEleitor3-1], candidatosMAP.get(candidatos[votoEleitor3-1])+1)
        } else {
...
function passo4(){
    ...
    console.log(vencedorVotos)
    console.log(vencedorCandidato)
